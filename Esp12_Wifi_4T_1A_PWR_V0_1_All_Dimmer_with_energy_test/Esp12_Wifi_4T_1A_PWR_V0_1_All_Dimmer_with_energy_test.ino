...
String DimmerState3 = "0";
String DimmerState4 = "0";
#endif  //Dimmer_State
...
void loop() {
  webSocket.loop();
   ...
   cTime  = millis();
 if (cTime - pTime >= delayTime)
 {
  Debugln("DEBUG:Configuration Flag HIGH");
  configured_count++;
  Debugln(configured_count);
  pTime = cTime;
  ...
  if(configured_count >= 6 && configured_count <= 12)
  {
    configured_flag = 0;
  }
  if(configured_count >12)
  {   
   configured_count = 0; 
  }
 }
/* 
  ...
  if(configured_count >= 5 && configured_count <= 10)
  {
    configured_flag == 0;
  }
 ...
 if ((millis() - last) > UPDATE_TIME) 
  ...
  espalexa.loop();
#endif
  ESP.wdtFeed();//added to for wdt on 11/11/2019 as per https://internetofhomethings.com/homethings/?p=396
 // Debugln("DEBUG: loop() end");
  delay(1);  //added to for wdt on 11/11/2019 as per https://internetofhomethings.com/homethings/?p=396
  //delay(1);
  //Debugln("DEBUG: loop() end");
}
